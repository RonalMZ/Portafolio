...
Abre **PowerShell** o **Windows Terminal** dentro de la carpeta `portfolio`:
...
1. Coloca la imagen en `public/projects/mi-proyecto.jpg`.
2. Usa `image: "/projects/mi-proyecto.jpg"`.
...
```
public/cv/Ronal-de-Jesus-CV.pdf
```

Si quieres otro nombre, actualízalo en `lib/data.ts` → `personalInfo.social.cv`.
...
│   └── projects/              # Imágenes locales de tus proyectos
├── package.json
├── tailwind.config.ts
├── postcss.config.mjs
├── next.config.mjs
...
¡Listo! Cualquier duda o sugerencia, escríbele directamente al dueño del portafolio desde la sección de Contacto.

...
function localFallback(userMessage: string): string {
  ...
  if (/ia|inteligencia|ai|chatbot|openai|gpt/.test(q)) {
    const aiProjects = projects.filter((p) =>
      p.tags.some((t) => /AI|OpenAI|Vercel AI/.test(t)),
    );
    return `${personalInfo.name} trabaja con IA usando el Vercel AI SDK y la API de OpenAI. Algunos ejemplos: ${aiProjects.map((p) => p.title).join(", ")}.`;
  }
  if (/e-?commerce|tienda|stripe|venta/.test(q)) {
    return `Tiene experiencia construyendo tiendas pequeñas con Next.js, integraciones tipo Stripe en modo test y bases de datos PostgreSQL. Un ejemplo es "Mercado Sinaloa".`;
  }
  if (/skill|habilidad|tecnolog|stack/.test(q)) {
    const top = skillCategories
      .map((c) => `${c.title}: ${c.skills.map((s) => s.name).slice(0, 3).join(", ")}`)
      .join(" · ");
    return `Su stack principal: ${top}.`;
  }
  if (/contact|email|correo|hire|contratar/.test(q)) {
    return `Puedes escribirle directo a ${personalInfo.email} o por LinkedIn. Está abierto a su primer rol profesional, remoto o en LATAM.`;
  }
  if (/ubicaci|donde|d[oó]nde|sinaloa|m[eé]xico/.test(q)) {
